...
gsigs = 'https://github.com/antfodaza/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('antdaza'):
        # Remove the potentially stale antdaza dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'antdaza', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'antdaza'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/antdaza/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'antdaza='+args.commit, '--url', 'antdaza='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/antdaza-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/antdaza/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/antdaza/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/antfodaza/antdaza', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/antdaza', exist_ok=True)
    os.chdir('builder/inputs/antdaza')
    if args.pull:
